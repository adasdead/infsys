...
static void add_cpu_section(struct ui_window *window)
{
    ...
    ui_textbox(window, features, 80, 60, 370, 80, UI_DEFAULT);
    ...
    ui_textbox(window, caches[CACHE_L1D], 80, 143, 140, 17, UI_DEFAULT);

    ui_label(window, "L1I Cache", 245, 145);
    ui_textbox(window, caches[CACHE_L1I], 310, 143, 140, 17, UI_DEFAULT);

    ui_label(window, "L2 Cache", 15, 165);
    ui_textbox(window, caches[CACHE_L2], 80, 163, 140, 17, UI_DEFAULT);

    ui_label(window, "L3 Cache", 245, 165);
    ui_textbox(window, caches[CACHE_L3], 310, 163, 140, 17, UI_DEFAULT);
}

static void add_gpu_section(struct ui_window *window)
{
...
static int infsys_run(void)
{
    ...
    window = ui_new_window(WINDOW_NAME, WINDOW_WIDTH, WINDOW_HEIGHT);

    add_cpu_section(window);
    ui_separator(window, 0, 200, 500);
    add_gpu_section(window);
